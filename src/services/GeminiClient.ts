...
export class GeminiClient implements LLMClient {
  ...
  constructor(apiKey: string) {
    if (!apiKey) {
      throw new Error('Gemini API key was not provided to the constructor.');
    }
...
`;
    ...
    try {
      ...
      const correlationResult = JSON.parse(jsonString);

      // TODO: Add validation to ensure the result matches the CorrelationResult interface
      return correlationResult as CorrelationResult;

    } catch (error) {
      console.error('Error correlating schemas with Gemini:', error);
      throw new Error('Failed to correlate schemas using Gemini.');
    }
  }
...
`;
    ...
    try {
      ...
      const response = result.response;
      const jsonString = response.text().replace(/```json|```/g, '').trim();
      const translatedTags = JSON.parse(jsonString);
      ...
      console.error('Error translating tags with Gemini:', error);
      throw new Error('Failed to translate tags using Gemini.');
    }
  }
} 